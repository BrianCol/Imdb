...
write_csv(x=linreg.preds, path="./LinearRegressionPredictionsColgrove.csv")

## Fit an tree model
## Fit an Elastic Net model
tree <- train(form=imdb_score~.,
               data=(imdb.train %>% select(-Set, -movie_title)),
               method="rpart",
               trControl=trainControl(method="repeatedcv",
                                      number=10, #Number of pieces of your data
                                      repeats=20) #repeats=1 = "cv"
)
plot(tree)

## Tune the elastic net
enet.grid <- expand.grid(cp=seq(0, .2 , length=20))
...
write_csv(x=elnet.preds, path="./TreePredictionsColgrove.csv")
